...
## 16. 施策5: ワイドフォーマット（1行=1breath）への学習方式変更

### 変更理由
`スコアアップ施策評価とロードマップ.md` の施策5に沿って、ロング形式の step 単位学習から、breath 全体の情報を同時に扱えるワイド形式へ切り替えるため。

### 変更内容
- **データセット作成の変更** … `u_out=0`（吸気相）を対象に `to_wide()` で `u_in_step*`, `u_out_step*` と `R`, `C` を生成し、目的変数を `pressure_step*` の多出力形式に変更
- **モデル学習の変更** … `MultiOutputRegressor(LGBMRegressor)` + `GroupKFold(5-fold)` で breath 単位学習へ変更
- **評価ロジックの変更** … fold ごとの MAE と OOF 予測を多出力前提で算出（`pressure_step*` をフラット化して MAE 計算）
- **提出処理の変更** … ワイド予測を long 形式（`breath_id`, `step`）へ復元し、`id` に再マップして提出。`u_out=1` は従来どおり `pressure=0` 補完
- **関連資料の追加** … 変更の目的・意図を整理した `施策5_変更内容の目的と変更意図.md` を追加
